...
[`dm_validate()`](https://dm.cynkra.com/reference/dm_validate.md) for
consistency.

`dm_add_tbl` is deprecated as of dm 1.0.0, because the same
functionality is offered by
[`dm()`](https://dm.cynkra.com/reference/dm.md) with
`.name_repair = "unique"`.

`dm_bind()` is deprecated as of dm 1.0.0, because the same functionality
is offered by [`dm()`](https://dm.cynkra.com/reference/dm.md).
