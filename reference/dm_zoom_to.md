...
Zooming to a table of a [`dm`](https://dm.cynkra.com/reference/dm.md)
allows for the use of many `dplyr`-verbs directly on this table, while
retaining the context of the `dm` object.
...
[`dm_filter()`](https://dm.cynkra.com/reference/dm_filter.md): only with
the latter, the filter expression is added to the list of table filters
stored in the dm.
