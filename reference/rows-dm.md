...
[`copy_dm_to()`](https://dm.cynkra.com/reference/copy_dm_to.md), no new
tables are created on the database. All operations expect that both
existing and new data are presented in two compatible
[dm](https://dm.cynkra.com/reference/dm.md) objects on the same data
source.
...
[`dm_ptype()`](https://dm.cynkra.com/reference/dm_ptype.md) as `x`. If
`in_place = TRUE`, the underlying data is updated as a side effect, and
...
#> Result is returned as a dm object with lazy tables. Use `in_place = FALSE` to
#> mute this message, or `in_place = TRUE` to write to the underlying tables.
#> ── Table source ────────────────────────────────────────────────────────────────
#> src:  sqlite 3.51.2 []
#> ── Metadata ────────────────────────────────────────────────────────────────────
