...
- When a local `dm` is copied to a database (DB) with
  [`copy_dm_to()`](https://dm.cynkra.com/reference/copy_dm_to.md), a PK
  will be set on the DB by default, whereas a UK is being ignored.
...
- A UK will be used, if the same table has an autoincrement PK in
  ...
  [`dm_rows_append()`](https://dm.cynkra.com/reference/rows-dm.md)) the
  foreign keys are updated accordingly. If no UK is available, the
