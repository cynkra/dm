...
- [Installation](#installation) explains how to install the package
...
- [Features](#features) gives a one-page overview of the scope of this package
...
- [Standing on the shoulders of giants](#standing-on-the-shoulders-of-giants) gives credit to related work


## Installation

The latest (development) version of {dm} can be installed from GitHub. It is not enough to just clone or download the package, you will need to use a package manager from your R session to configure necessary dependencies and to bind file locations to your local environment: 
...
The {dm} package will also be made available on [CRAN](https://cran.r-project.org/web/packages/), where it can be installed with the command
...
The motivation for creating the {dm} package is to provide more sophisticated data management for R users.
...
This dataset contains five tables: the main `flights` table with links to the `airlines`, `planes` and `airports` tables, and a `weather` table without an explicit link.
...
In contrast, using the classical {dplyr} notation, you will need three `left_join()` calls to merge the `flights` table gradually to `airlines`, `planes` and `airports` tables to create one wide data frame.
...
You can find more information to jump-start working with {dm} in the article ["Introduction to Relational Data Models"](https://krlmlr.github.io/dm/articles/dm-introduction-relational-data-models.html).

### The Advantages in Brief

The separation of one large table into multiple tables achieves several goals:

- **Avoid repetition, conserve memory**: all information needs to be stored only once
    - the name of each airline
    - the name, location and altitude of each airport
    - the manufacturer and number of seats for each airplane
- **Improve consistency**: all information needs to be updated only once and in one place, avoiding errors that  result from inconsistent updating in different tables
...
The {dm} package helps with many challenges that arise with working with relational data models.
