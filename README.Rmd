...
In classical {dplyr} notation you need three `left_join()` calls to merge the `flights` table gradually to `airlines`, `planes` and `airports` tables to create one big dataframe.
...
In contrast, **using a relational data model shortens the task** to one line of code by using `cdm_flatten_to_tbl()`.

The reason: A once defined data model contains all relations between the tables.
The function `cdm_nycflights13()` helps to do this with the example data.
After creating such [a `dm` object](https://krlmlr.github.io/dm/articles/dm-class-and-basic-operations.html), you can use these links as often as you wish - without explicitly referring to the relations ever again.
The task of joining `flights` with `airlines`, `planes` and `airports` boils down to:
...
### The Case for Relational Data Models for {dplyr} Users
...
Let's illustrate this challenge with the `flights` data.
Details like the full name of an airport are not available immediately but must be joined/merged.
This can result in long and inflated pipe chains full of `left_join()`, `anti_join()` and other  forms of merging data ([see Example above](#example)).

{dm} offers a more elegant and shorter way to combine values by establishing key relations (see next section [Good to Know](#good-to-know)) while augmenting {dplyr}/{dbplyr} workflows.
...
Normalization is the technical term that describes the central design principle of a relational data model: splitting data into multiple tables.
