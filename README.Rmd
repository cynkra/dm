...
In contrast, using the classical {dplyr} notation you need three `left_join()` calls to merge the `flights` table gradually to `airlines`, `planes` and `airports` tables to create one wide dataframe.
...
Flat file systems like spreadsheets and dataframes can result in bloated tables, that hold many repetitive values.
