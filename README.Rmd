...
For the example data set, you can use `cdm_nycflights13()` to create the `dm` object, and a single command for merging the tables.
...
For data other than the example data, the `dm` object would need to be created by using the `dm()` constructor and by adding keys using `cdm_add_pk()` and `cdm_add_fk()`.
