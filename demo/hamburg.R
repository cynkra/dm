...
dm_flights<-dm_nycflights13(cycle=T)
...
nycflights13_fk <-
  ...
  dm_add_fk(flights,carrier,airlines,check=T)
...
dm_flights_sqlite <-
  ...
  copy_dm_to(
    dbplyr::src_memdb(),
    .,
    unique_table_names=T,
    set_key_constraints=F
)
...
try({  
  ...
  dm_flights_pq <-
    dm_flights_small %>%
    copy_dm_to(con_pq,.,temporary=F,table_names=qualified_names)  
  dm_flights_from_pq <-
