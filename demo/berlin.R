...
# FIXME: remove all_connected=T for now,redo example above
...
nycflights13_sqlite <-
  ...
  dm_filter(flights,month==1)%>% 
  dm_apply_filters() %>%
  copy_dm_to(dbplyr::src_memdb(),.,unique_table_names=T)
nycflights13_sqlite
...
# It's tricky:
weather %>%
  unite("slot_id",origin,year,month,day,hour,remove=F)%>% 
  count(slot_id) %>%
  filter(n > 1)
weather%>% 
...
weather_link <-
  ...
  unite("origin_slot_id",origin,time_hour_fmt,remove=F)
...
nycflights13_fk <-
  nycflights13_pk %>%
  dm_add_fk(flights,origin_slot_id,weather,check=F) %>%
  dm_add_fk(flights,tailnum,planes,check=F) %>%
  dm_add_fk(flights,origin,airports) %>%
  dm_add_fk(flights,dest,airports,check=F) %>%
...
try({  
  ...
  dm_pq<- 
    ...
    dm_select_tbl(-planes) %>%
    dm_filter(flights,month==1) %>%
    copy_dm_to(src_postgres(),.,temporary=F)
