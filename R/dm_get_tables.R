#' Get tables
#'
#' `dm_get_tables()` returns a named list of \pkg{dplyr} [tbl] objects of a `dm` object.
#'
#' @param x A `dm` object.
#' @inheritParams rlang::args_dots_empty
#' @param keyed `r lifecycle::badge("experimental")`
#'   Set to `TRUE` to return objects of the internal class `"dm_keyed_tbl"`
#'   that will contain information on primary and foreign keys in the individual table objects.
#'   This allows using dplyr workflows on those tables and later reconstruct them into a `dm` object.
...
#'
#' @return A named list with the tables (data frames or lazy tables) constituting the `dm`.
#'
