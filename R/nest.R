nest_join_dm_zoomed <- function(x, ...) {
  ...
  child_tables <-
    ...
    dplyr::mutate(data = map(child_table, ~ dm_get_tables_impl(dm_zoomed)[[.x]])) %>%
    # FIXME: should we check and warn/message, if no child table is in selected?
    dplyr::filter(child_table %in% selected) %>%
    # perform joins in the order given in the ellipsis
    dplyr::arrange(match(child_table, selected))
  ...
  for (i in seq_len(nrow(child_tables))) {
    x <- dplyr::nest_join(
      ...
      dplyr::mutate(!!child_tables$child_table[i] := as_list_of(!!sym(child_tables$child_table[i])))
  }
