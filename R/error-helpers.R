...
abort_not_unique_key <- function(table_name, column_names) {
  abort(
    error_txt_not_unique_key(table_name, column_names),
...
abort_sets_not_equal <- function(error_msgs) {
  abort(
    error_txt_sets_not_equal(error_msgs),
    class = dm_error_full("sets_not_equal")
  )
}
...
abort_ref_tbl_has_no_pk <- function(ref_table_name) {
  abort(
    error_txt_ref_tbl_has_no_pk(ref_table_name),
...
abort_no_cycles <- function(g) {
  ...
  shortest_cycle <- paste(
    c(shortest_cycle, shortest_cycle[1]),
    collapse = " -> "
  )
...
abort_dupl_new_id_col_name <- function(table_name) {
  abort(
    error_txt_dupl_new_id_col_name(table_name),
...
error_txt_dupl_new_id_col_name <- function(table_name) {
  glue(
    "`new_id_column` can't have an identical name as one of the columns of {tick(table_name)}."
  )
}
...
error_txt_tables_not_neighbors <- function(t1_name, t2_name) {
  glue(
    "Tables `{t1_name}` and `{t2_name}` are not directly linked by a foreign key relation."
  )
}
...
abort_what_a_weird_object <- function(class) {
  abort(
    error_txt_what_a_weird_object(class),
...
error_txt_what_a_weird_object <- function(class) {
  glue(
    "Don't know how to determine table source for object of class {commas(tick(class))}."
  )
}
...
abort_only_possible_wo_zoom <- function(fun_name) {
  abort(
    error_txt_only_possible_wo_zoom(fun_name),
...
abort_only_possible_w_zoom <- function(fun_name) {
  abort(
    error_txt_only_possible_w_zoom(fun_name),
...
error_txt_only_possible_w_zoom <- function(fun_name) {
  glue(
    "You can't call `{fun_name}()` on an unzoomed `dm`. Consider using `dm_zoom_to()` first."
  )
}
...
abort_fk_not_tracked <- function(x_orig_name, y_name) {
  abort(
    error_txt_fk_not_tracked(x_orig_name, y_name),
...
abort_pk_not_tracked <- function(orig_table, orig_pk) {
  abort(
    error_txt_pk_not_tracked(orig_table, orig_pk),
...
abort_cols_not_avail <- function(wrong_col) {
  abort(
    error_txt_cols_not_avail(wrong_col),
...
abort_only_named_args <- function(fun_name, name_meaning) {
  abort(
    error_txt_only_named_args(fun_name, name_meaning),
...
error_txt_parameter_not_correct_class <- function(
  parameter,
  correct_class,
  class
) {
...
abort_parameter_not_correct_length <- function(
  parameter,
  correct_length,
  parameter_value
) {
...
error_txt_parameter_not_correct_length <- function(
  parameter,
  correct_length,
  parameter_value
) {
