...
dm_ddl_pre <- function(dm, dest, temporary=TRUE, table_names=set_names(names(dm)), set_key_constraints=TRUE, schema=NULL) {
  ...
  stopifnot(length(dm)==length(names(dm)), length(dm)==length(pks), length(dm)==length(ais),
...
dml_cols <- function(x, dest, pk, autoincrement) {
  character()
}
dml_vals <- function(x, dest, pk, autoincrement) {
  character()
}

#' @rdname dm_sql
#' @export
dm_dml_load <- function(dm, dest, table_names=set_names(names(dm)), schema=NULL) {
  pk <- 1
  autoincrement <- 1
  # for each tbl
  cols <- paste(dml_cols(dm, dest, pk, autoincrement), collapse=", ")
  vals <- paste(dml_vals(dm, dest, pk, autoincrement), collapse=", ")
  #...
  character()
}
