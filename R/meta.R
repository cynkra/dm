...
dm_meta_raw <- function(con, catalog) {
  ...
  if (is_mssql(src)) {
    ...
    cli::cli_alert_warning(
      "unable to fetch autoincrement metadata for src '{class(src)[1]}'"
    )
    columns <- columns %>%
  ...
  if (is_postgres(src)) {
    ...
    constraint_column_usage <- mssql_constraint_column_usage(
      src,
      table_constraints,
      catalog
    )
...
filter_dm_meta <- function(dm_meta, catalog = NULL, schema = NULL) {
  ...
  if (length(schema) > 1 && anyNA(schema)) {
    cli::cli_abort(
      "{.arg schema} must not contain NA if it has more than one element."
    )
  }
  ...
  if (!is.null(catalog) && !is.na(catalog)) {
    ...
    table_constraints <- table_constraints %>%
      filter(table_catalog %in% !!catalog)
...
filter_dm_meta_simple <- function(dm_meta, catalog = NULL, schema = NULL) {
  ...
  if (!is.null(schema)) {
    ...
    !is.null(schema) && !is.na(schema) && is_mariadb(dm_get_con(dm_meta))
  ) {
    schemata <- schemata %>%
      filter(schema_name == DATABASE() | is.na(DATABASE()))
    tables <- tables %>% filter(table_schema == DATABASE() | is.na(DATABASE()))
