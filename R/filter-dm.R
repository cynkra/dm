...
get_all_filtered_connected <- function(dm, table) {
  ...
  # the predecessor.
  distances <- dm_distances(graph, table)[1, ]
  ...
  graph <- dm_induced_subgraph(graph, target_tables)
  if (length(dm_E(graph)) >= length(dm_V(graph))) {
    abort_no_cycles(graph)
  }
  paths <- dm_shortest_paths(graph, table, target_tables, predecessors = TRUE)

  # All edges with finite distance as tidy data frame
  all_edges <-
    new_filtered_edges(
      node = names(dm_V(graph)),
