...
#'
#' @export
dm_add_fk <- nse_function(c(dm, table, column, ref_table, check = FALSE), ~ {
...
#'
#' @export
dm_get_all_fks <- nse_function(c(dm), ~ {
...
#'
#' @details `dm_enum_fk_candidates()` checks first, if `ref_table` has a primary key set.
...
#'
#' @export
dm_enum_fk_candidates <- nse_function(c(dm, table, ref_table), ~ {
  check_no_filter(dm)
