...
dm_add_fk <- function(
  ...
  if (is.null(ref_col_expr)) {
    ...
    ref_col_name <- names(eval_select_indices(
      ref_col_expr,
      colnames(ref_table_obj)
    ))
  }
...
dm_add_fk_impl <- function(
  ...
  if (n_loc > 1) {
    ...
    dm <- dm_add_fk_impl(
      ...
      table[where_other],
      column[where_other],
      ref_table[where_other],
      ref_column[where_other],
      on_delete[where_other]
    )
...
dm_rm_fk_impl <- function(dm, table_name, cols, ref_table_name, ref_cols) {
  ...
  if (!is.null(ref_cols)) {
    idx_fk <- pmap(
      list(def$fks[idx], idx_fk, def$data[idx]),
      ~ {
        ii <- tryCatch(
          {
  ...
  if (show_disambiguation) {
    ...
    disambiguation <-
      ...
      unnest(-ref_table) %>%
      mutate(
        ref_col_text = if_else(
          need_ref,
          glue(", {deparse_keys(ref_column)})"),
          ""
        )
      ) %>%
      mutate(
...
#' @autoglobal
enum_fk_candidates_impl <- function(
  table_name,
  tbl,
  ref_table_name,
  ref_tbl,
  ...
  tibble(
    ...
    why = map_chr(
      column,
      ~ check_fk(tbl, table_name, .x, ref_tbl, ref_table_name, ref_tbl_cols)
    )
  ) %>%
