...
- Keep a set of related tables, their primary and foreign keys, and their descriptions together in one object,
  which behaves like a named list of tables.
- Import an existing data model from a database in a single call to `dm_from_con()`,
  including the keys for the backends that expose them.
- Check a model against the data it holds:
  key uniqueness, referential integrity, and cardinalities between tables.
- Draw the model as a diagram,
  and use the recorded keys to join and flatten tables without restating the join conditions every time.
- Deploy a model to an RDBMS with `copy_dm_to()`,
  and insert, update or delete rows in place with the `dm_rows_*()` verbs.

It is explicitly not trying to:

- Be a database:
  dm describes and manipulates a relational data model, the data itself lives in your R session or in an RDBMS.
- Talk to a database on its own:
  connections come from a {DBI} backend, and the SQL for lazy tables is generated by {dbplyr}.
- Read data out of files:
  csv files and spreadsheets have to be imported to data frames before they can enter a dm.
- Span more than one source:
