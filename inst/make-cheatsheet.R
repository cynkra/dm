...
withr::with_dir(
  temp_folder,
  {
    ...
    brio::write_lines(
      original[-c((length(original) - 4):length(original))],
      rmd_path
    )
    ...
    subtitle <- xml2::xml_find_first(html, ".//blockquote/p") |>
      xml2::xml_text()
    ...
    fs::dir_create(file.path("cheatsheet", "deps"))
    deps <- lapply(
      deps,
      htmltools::copyDependencyToDir,
      file.path("cheatsheet", "deps")
    )
    ...
    # logo -------
    fs::dir_copy(
      file.path(input_dir, "cheatsheet-figures"),
      "cheatsheet-figures"
    )
