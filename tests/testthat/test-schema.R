test_that("schema handling on MSSQL and Postgres works",{
  ...
  expect_identical(
    con_db %>%
      db_schema_list(include_default=F) %>%
...
test_that("schema handling on Postgres works",{
  ...
  expect_identical(
    sort(
      filter(
        ...
        db_schema_list(con_db,include_default=T),  
  ...
  expect_message(
    db_schema_drop(con_db,"2-dm_schema_TEST",force=T),
...
test_that("schema handling on MSSQL works for different DBs",{
  ...
  expect_identical(
    sort(
      db_schema_list(con_db,include_default=T,dbname="test_db_for_schema_dm")$schema_name
),
    c("dbo","test_schema")
)
  expect_message(
  ...
  expect_error(
    expect_warning(
      db_schema_drop(con_db,"test_schema",dbname="test_db_for_schema_dm",force=T),
