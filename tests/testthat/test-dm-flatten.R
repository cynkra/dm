...
test_that("`dm_flatten()` recursive snapshot with `dm_paste()`",{
  expect_snapshot({  
    dm_flatten(dm_more_complex(),tf_5,parent_tables=c(tf_4,tf_3),recursive=T) %>%
      dm_paste(options=c("select","keys"))
})
})
...
test_that("`dm_flatten()` allow_deep snapshot with `dm_paste()`",{
  ...
  expect_snapshot({
    dm_flatten(dm_deep,A,allow_deep=T) %>%
      dm_paste(options=c("select","keys"))
})
})
...
test_that("`dm_flatten()` with recursive=T works",{  
  # dm_more_complex has tf_5 -> tf_4 -> tf_3
  result<-dm_flatten(dm_more_complex(),tf_5,parent_tables=c(tf_4,tf_3),recursive=T)  
  expect_s3_class(result,"dm")
...
test_that("`dm_flatten()` with recursive=T auto-detect",{
  result<-dm_flatten(dm_more_complex(),tf_5,recursive=T)
...
test_that("`dm_flatten()` with allow_deep=T keeps grandparents",{
  dm_deep<-dm_for_flatten_deep()

  result<-dm_flatten(dm_deep,A,allow_deep=T)
...
test_that("`dm_flatten()` errors when recursive and allow_deep are both T",{
  expect_dm_error(
    dm_flatten(dm_for_flatten(),fact,recursive=T,allow_deep=T),
...
test_that("`dm_flatten_impl()` recurses with dfs_order",{
  ...
  g<-create_graph_from_dm(dm_deep,directed=T)
  g_sub<-graph_induced_subgraph(g,c("A","B","C"))
  dfs<-graph_dfs(g_sub,"A",unreachable=F,dist=T)
...
test_that("`dm_flatten_transfer_fks()` re-points FKs to start table",{
  dm_deep<-dm_for_flatten_deep()
  all_fks<-dm_get_all_fks_impl(dm_deep,ignore_on_delete=T)
...
test_that("`dm_flatten_transfer_fks()` handles renamed FK columns",{
  ...
  all_fks<-dm_get_all_fks_impl(dm_test,ignore_on_delete=T)
  out<-dm_flatten_join(dm_test,"child","parent",left_join)
...
test_that("`dm_flatten()` recursive with non-left join errors",{
  expect_dm_error(
    dm_flatten(dm_more_complex(),tf_5,recursive=T,join=semi_join),
