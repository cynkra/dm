...
---
    ...
    (`year`, `manufacturer`, and `model`) not a unique key of `planes`.
...
---
    ...
    A UK (`year`, `month`, `day`, `dep_time`, `sched_dep_time`, `dep_delay`, `arr_time`, `sched_arr_time`, `arr_delay`, `carrier`, `flight`, `tailnum`, `origin`, `dest`, `air_time`, `distance`, `hour`, `minute`, and `time_hour`) for table `flights` already exists, not adding UK.
