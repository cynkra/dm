# build_copy_queries snapshot test for pixarfilms
    ...
    Output
      ...
      SELECT
        ...
        CAST("run_time" AS NUMERIC) AS "run_time",
        CAST("film_rating" AS TEXT) AS "film_rating"
      FROM (
        VALUES
          ('1', 'Toy Story', '1995-11-22'::date, 81.0, 'G'),
          ('2', 'A Bug''s Life', '1998-11-25'::date, 95.0, 'G'),
          ('3', 'Toy Story 2', '1999-11-24'::date, 92.0, 'G'),
          ('4', 'Monsters, Inc.', '2001-11-02'::date, 92.0, 'G'),
          ('5', 'Finding Nemo', '2003-05-30'::date, 100.0, 'G'),
          ('6', 'The Incredibles', '2004-11-05'::date, 115.0, 'PG'),
          ('7', 'Cars', '2006-06-09'::date, 117.0, 'G'),
          ('8', 'Ratatouille', '2007-06-29'::date, 111.0, 'G'),
          ('9', 'WALL-E', '2008-06-27'::date, 98.0, 'G'),
          ('10', 'Up', '2009-05-29'::date, 96.0, 'PG'),
          ('11', 'Toy Story 3', '2010-06-18'::date, 103.0, 'G'),
          ('12', 'Cars 2', '2011-06-24'::date, 106.0, 'G'),
          ('13', 'Brave', '2012-06-22'::date, 93.0, 'PG'),
          ('14', 'Monsters University', '2013-06-21'::date, 104.0, 'G'),
          ('15', 'Inside Out', '2015-06-19'::date, 95.0, 'PG'),
          ('16', 'The Good Dinosaur', '2015-11-25'::date, 93.0, 'PG'),
          ('17', 'Finding Dory', '2016-06-17'::date, 97.0, 'PG'),
          ('18', 'Cars 3', '2017-06-16'::date, 102.0, 'G'),
          ('19', 'Coco', '2017-11-22'::date, 105.0, 'PG'),
          ('20', 'Incredibles 2', '2018-06-15'::date, 118.0, 'PG'),
          ('21', 'Toy Story 4', '2019-06-21'::date, 100.0, 'G'),
          ('22', 'Onward', '2020-03-06'::date, 102.0, 'PG'),
          ('23', 'Soul', '2020-12-25'::date, 100.0, 'PG'),
          ('24', 'Luca', '2021-06-18'::date, 151.0, 'N/A'),
          ('25', 'Turning Red', '2022-03-11'::date, NULL, 'N/A'),
          ('26', 'Lightyear', '2022-06-17'::date, NULL, 'N/A')
      ) AS drvd("number", "film", "release_date", "run_time", "film_rating")
      ...
      ) AS drvd("film", "award_type", "status")
      ...
      SELECT
        ...
        CAST("box_office_us_canada" AS NUMERIC) AS "box_office_us_canada",
        CAST("box_office_other" AS NUMERIC) AS "box_office_other",
        CAST("box_office_worldwide" AS NUMERIC) AS "box_office_worldwide"
      FROM (
        VALUES
      ...
      ) AS drvd("film", "budget", "box_office_us_canada", "box_office_other", "box_office_worldwide")
      <SQL> INSERT INTO "public_response" ("film", "rotten_tomatoes", "metacritic", "cinema_score", "critics_choice")
      SELECT
        CAST("film" AS TEXT) AS "film",
        CAST("rotten_tomatoes" AS NUMERIC) AS "rotten_tomatoes",
        CAST("metacritic" AS NUMERIC) AS "metacritic",
        CAST("cinema_score" AS TEXT) AS "cinema_score",
        CAST("critics_choice" AS NUMERIC) AS "critics_choice"
      FROM (
        VALUES
          ...
          ('Soul', 96.0, 83.0, NULL::text, 93.0),
          ('Luca', NULL, NULL, NULL::text, NULL)
      ) AS drvd("film", "rotten_tomatoes", "metacritic", "cinema_score", "critics_choice")
...
# build_copy_queries avoids duplicate indexes
    ...
    Output
      ...
      SELECT CAST("a__key" AS NUMERIC) AS "a__key"
      FROM (  VALUES (1.0)) AS drvd("a__key")
      <SQL> INSERT INTO "child__a" ("key")
      SELECT CAST("key" AS NUMERIC) AS "key"
      FROM (  VALUES (1.0)) AS drvd("key")
