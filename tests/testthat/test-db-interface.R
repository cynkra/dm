...
# FIXME: Add test that set_key_constraints=F doesn't set key constraints,
# in combination with dm_learn_from_db


test_that("copy_dm_to() rejects overwrite and types arguments",{  
  expect_snapshot(error=T,{
    copy_dm_to(my_test_src(),dm_for_filter(),overwrite=T)
})
  expect_snapshot(error=T,{
...
test_that("default table repair works",{
  ...
  repair_table_names_for_db(table_names,temporary=F,con)
  expect_equal(calls,0)
  repair_table_names_for_db(table_names,temporary=T,con)
  expect_gt(calls,0)
})
...
test_that("copy_dm_to() fails legibly if target schema missing for MSSQL & Postgres",{
  ...
  expect_error(
    copy_dm_to(src_db,local_dm,schema="copy_dm_to_schema",temporary=F)
)
})
...
test_that("copy_dm_to(table_names=) (#250)",{
  ...
  withr::defer({
    try(DBI::dbExecute(my_db_test_con(),"DROP TABLE y",immediate=T))
})
  ...
  withr::defer({
    try(DBI::dbExecute(my_db_test_con(),"DROP TABLE x1",immediate=T))
    try(DBI::dbExecute(my_db_test_con(),"DROP TABLE y1",immediate=T))
})
