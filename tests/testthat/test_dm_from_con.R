test_that("table identifiers are quoted",{
  ...
  dm <-
    suppress_mssql_warning(dm_from_con(con_db,learn_keys=F)) %>%
    dm_select_tbl(
      !!!map(
...
test_that("table identifiers are quoted with learn_keys=F",{
  con_db<-my_db_test_con()
  ...
  dm<-suppress_mssql_warning(dm_from_con(con_from_src_or_con(con_db),learn_keys=F))
  remote_names <-
    dm %>%
    dm_get_tables() %>%
    map_chr(remote_name_qual)
...
test_that("dm_from_src() deprecated",{
  ...
  expect_deprecated(dm_from_src(src_from_src_or_con(con_db),learn_keys=F))
})
