...
test_that("dm_apply_filters_impl optimization works correctly", {
  ...
  dm_mixed <- dm(
    ...
    connected_2 = tibble(id = 1:5, ref_id = 1:5, value = LETTERS[1:5]),
    isolated_1 = tibble(id = 1:8, value = paste0("iso1_", 1:8)),
    isolated_2 = tibble(id = 1:3, value = paste0("iso2_", 1:3))
  ) %>%
    dm_add_pk(connected_1, id) %>%
    dm_add_pk(connected_2, id) %>%
