...
test_that("dm_rows_insert()",{
  ...
  # Target database:
  flights_sqlite<-copy_dm_to(sqlite,flights_init,temporary=F)
  ...
  expect_snapshot({
    ...
    dm_rows_append(flights_sqlite,flights_hour10_sqlite,in_place=T)
    print(dm_nrow(flights_sqlite))
    ...
    # Apply:  
    dm_rows_append(flights_sqlite,flights_hour11_sqlite,in_place=T)
    print(dm_nrow(flights_sqlite))
...
test_that("dm_rows_update()",{
  ...
  expect_snapshot({
    ...
    dm_copy %>%
      dm_rows_update(dm_update_copy,in_place=F) %>%
      pull_tbl(tf_2) %>%
      arrange_all()
    ...
    dm_copy %>%
      dm_rows_update(dm_update_copy,in_place=T)
    dm_copy %>%
      dm_get_tables() %>%
      map(arrange_all)
})
...
test_that("dm_rows_truncate()",{
  ...
  expect_snapshot({
    ...
    dm_copy %>%
      ...
      arrange_all()
    dm_copy %>%
      dm_rows_truncate(dm_truncate_copy,in_place=F) %>%
      pull_tbl(tf_2) %>%
      arrange_all()
    ...
    dm_copy %>%
      ...
      map(arrange_all)
    dm_copy %>%
      dm_rows_truncate(dm_truncate_copy,in_place=T)
...
test_that("output for compound keys",{
  ...
  expect_snapshot({
    ...
    insert_dm <-
      ...
      dm_apply_filters() %>%
      dm_select_tbl(flights,weather)
    dm_rows_insert(target_dm,insert_dm,in_place=F)
    dm_rows_truncate(nyc_comp(),insert_dm,in_place=F)
...
test_that("dm_rows_append() works with autoincrement PKs and FKS for selected DBs",{
  ...
  dm_ai_w_keys <-
    dm_for_autoinc_1() %>%
    dm_add_pk(t1,a,autoincrement=T) %>%
    dm_add_pk(t2,c,autoincrement=T) %>%
    dm_add_pk(t4,g,autoincrement=T) %>%
    dm_add_fk(t2,d,t1) %>%
