test_that("Standard learning from MSSQL (schema 'dbo') or Postgres (schema 'public') and get_src_tbl_names() works?",{
  ...
  expect_silent(dm_db_learned_all<-dm_from_con(con_db,learn_keys=T))
  ...
  expect_equivalent_dm(
    ...
    ignore_on_delete=F,
    ignore_autoincrement=F
)
  # learning without keys:
  expect_silent(dm_db_learned_no_keys<-dm_from_con(con_db,learn_keys=F))
...
test_that("Learning from specific schema works?",{
  ...
  random_schema<-function() {
    reservoir<-c(letters)
    how_long<-10  
    paste0(reservoir[sample(seq_len(length(reservoir)),how_long,replace=T)],collapse="")
}
  ...
  # learning with keys:
  dm_db_learned <-
    dm_from_con(con_db,schema=schema_name,learn_keys=T) %>%
  ...
  # learning without keys:  
  dm_db_learned_no_keys <-
    expect_silent(dm_from_con(con_db,schema=schema_name,learn_keys=F)) %>%
    dm_select_tbl(!!!order_of_deletion)
...
test_that("Learning keys from SQLite works",{
  ...
  learned_dm<-dm_from_con(con_sqlite,learn_keys=T) %>%
    collect()
...
test_that("dm_meta() contents",{
  ...
  constraints<-dm_examine_constraints(meta,.progress=F)
...
test_that("dm_from_con() with mariaDB",{
  ...
  my_dm<-dm_from_con(my_db,learn_keys=T)
