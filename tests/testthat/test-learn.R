test_that("Standard learning from MSSQL (schema 'dbo') or Postgres (schema 'public') and get_src_tbl_names() works?", {
  ...
  remote_tbl_map <- set_names(
    remote_tbl_names,
    gsub("^(tf_.).*$", "\\1", remote_tbl_names)
  )
  ...
  expect_silent(
    dm_db_learned_no_keys <- dm_from_con(con_db, learn_keys = FALSE)
  )
...
test_that("Learning from specific schema works?", {
  ...
  random_schema <- function() {
    ...
    how_long <- 10
    paste0(
      reservoir[sample(seq_len(length(reservoir)), how_long, replace = TRUE)],
      collapse = ""
    )
  }
  ...
  remote_tbl_names <- set_names(
    paste0(
      schema_name_q,
      ".",
      DBI::dbQuoteIdentifier(con_db, order_of_deletion)
    ),
    order_of_deletion
  )
...
test_that("Learning from MSSQL (schema 'dbo') on other DB works?", {
  ...
  DBI::dbExecute(
    con_db,
    "ALTER TABLE [test_database_dm].[dbo].[test_1] ADD PRIMARY KEY ([b])"
  )
  # set FK relation
  DBI::dbExecute(
    con_db,
...
test_that("Learning from a specific schema in another DB for MSSQL works?", {
  ...
  DBI::dbExecute(
    con_db,
    "ALTER TABLE [test_database_dm].[dm_test].[test_1] ADD PRIMARY KEY ([b])"
  )
  # set FK relation
  DBI::dbExecute(
    con_db,
...
test_that("dm_meta() contents", {
  ...
  withr::defer({
    try(walk(
      ...
      ~ try(DBI::dbExecute(
        con_db,
        paste0("DROP TABLE ", schema_name_q, ".", .x, " CASCADE")
      ))
    ))
    try(DBI::dbExecute(con_db, paste0("DROP SCHEMA ", schema_name_q)))
  })
    ...
    meta %>%
      ...
      map(
        ~ if ("constraint_name" %in% colnames(.x)) {
          .x %>%
            mutate(
              constraint_name = as.integer(forcats::fct_inorder(
                constraint_name
              ))
            )
        } else {
