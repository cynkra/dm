...
local({
  ...
  my_test_src_cache <<- function() {
    tryCatch(
      my_test_src_fun()(),
      error = function(e) {
        skip(paste0("Data source postgres not accessible: ", conditionMessage(e)))
      }
    )
  }
  ...
  my_test_src <<- function() {
    ...
    fun <- my_test_src_fun()
    if (is.null(fun)) {
      skip(paste0("Data source not known: postgres"))
    }
    my_test_src_cache()
  }
})
