...
test_that("'nest_join()'-method for `zoomed_dm` works", {
  expect_identical(
    zoomed_dm_2 %>%
      nest_join() %>%
      pull_tbl(),
  ...
  expect_identical(
    ...
    expect_message(
      expect_equivalent_dm(
        dm_rm_pk(dm_for_filter, t3, TRUE) %>% dm_zoom_to(t3) %>% nest_join(),
    ...
    expect_dm_error(
      zoomed_dm_2 %>% select(g) %>% nest_join(),
      "pk_not_tracked"
    )
})

test_that("'nest_join.zoomed_dm()' fails for Postgres-'dm'", {
  skip_if_not("postgres" %in% src_names)
  expect_dm_error(
    dm_zoom_to(dm_for_filter_src$postgres, t3) %>% nest_join(),
    "only_for_local_src"
  )
})

test_that("'nest_join.zoomed_dm()' fails for SQLite-'dm'", {
  skip_if_not("sqlite" %in% src_names)
  expect_dm_error(
    dm_zoom_to(dm_for_filter_src$sqlite, t3) %>% nest_join(),
    "only_for_local_src"
  )
})
