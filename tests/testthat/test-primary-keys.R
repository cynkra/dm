test_that("dm_add_pk() works as intended?",{
  ...
  expect_silent(expect_equivalent_dm(  
    dm_test_obj() %>%
      ...
      dm_add_pk(dm_table_1,a) %>%  
      dm_add_pk(dm_table_1,b,force=T),
    dm_test_obj() %>%
  ...
  expect_dm_error(
    dm_add_pk(dm_test_obj(),dm_table_2,c,check=T),
...
test_that("dm_rm_pk() supports partial filters",{
  expect_snapshot({
    # test logic if argument `fail_fk=F`
    dm_for_filter() %>%
      dm_rm_pk(tf_4) %>%
      get_all_keys()
    ...
    dm_for_filter() %>%
      dm_rm_pk(tf_4,rm_referencing_fks=T) %>%
      get_all_keys()
...
test_that("autoincrement fails with compound keys",{  
  expect_snapshot(error=T,{
    dm(x) %>%
      dm_add_pk(x,columns=c(x_id,z),autoincrement=T)
})
})
...
test_that("set autoincrement PK",{
  expect_snapshot({
    dm(x,y=x) %>%
      dm_add_pk(x,columns=c(x_id),autoincrement=T) %>%
