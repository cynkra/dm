...
test_that("can upgrade zoomed from v1",{
  ...
  expect_snapshot({
    def<-dm_get_def(dm_v1_zoomed,quiet=T)
    def<-dm_get_def(dm_v1_zoomed)
    dm<-dm_from_def(def,zoomed=T)
    dm_validate(dm)
    is_zoomed(dm)
})
})
...
test_that("can upgrade zoomed from v2",{
  ...
  expect_snapshot({
    def<-dm_get_def(dm_v2_zoomed,quiet=T)
...
test_that("can upgrade zoomed from v3",{
  ...
  expect_snapshot({
    def<-dm_get_def(dm_v3_zoomed,quiet=T)
...
test_that("can upgrade zoomed to v4",{
  ...
  expect_snapshot({
    def<-dm_get_def(dm_v4_zoomed,quiet=T)
