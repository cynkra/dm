...
test_that("basic test: 'summarise()'-methods work",{
  expect_equivalent_tbl(
    summarise(dm_zoomed(),d_2=mean(d,na.rm=T))%>%tbl_zoomed(),
    summarise(tf_2(),d_2=mean(d,na.rm=T))
...
test_that("basic test: 'count()'-method works",{
  ...
  expect_equivalent_tbl(
    count(dm_zoomed(),sort=T)%>%tbl_zoomed(),
    count(tf_2(),sort=T)
)
...
test_that("basic test: 'arrange()'-methods work",{
  ...
  expect_equivalent_tbl(
    group_by(dm_zoomed(),e)%>%arrange(desc(d),.by_group=T)%>%tbl_zoomed(),
    arrange(group_by(tf_2(),e),desc(d),.by_group=T)
)
...
test_that("basic test: 'slice()'-methods work",{  
  ...
  expect_silent(
    expect_equivalent_tbl(
      slice(dm_zoomed(),if_else(d < 5,1:6,7:2),.keep_pk=F)%>%tbl_zoomed(),  
...
test_that("key tracking works for slice()",{
  ...
  expect_identical(  
    slice(dm_zoomed(),if_else(d < 5,1:6,7:2),.keep_pk=F)%>%col_tracker_zoomed(),
    set_names(c("d","e","e1"))
)
  ...
  expect_identical(  
    slice(dm_zoomed(),if_else(d < 5,1:6,7:2),.keep_pk=T)%>%col_tracker_zoomed(),
    set_names(c("c","d","e","e1"))
)
})
...
test_that("'summarize_at()' etc. work",{
  expect_equivalent_tbl(
    dm_nycflights_small() %>%
      dm_zoom_to(airports) %>%
      summarize_at(vars(lat,lon),list(mean=mean,min=min,max=max),na.rm=T) %>%
      tbl_zoomed(),
    dm_nycflights_small() %>%
      pull_tbl(airports) %>%
      summarize_at(vars(lat,lon),list(mean=mean,min=min,max=max),na.rm=T)
  ...
  expect_equivalent_tbl(
    dm_nycflights_small() %>%
      dm_zoom_to(airports) %>%
      select(3:6) %>%  
      summarize_all(list(mean=mean,sum=sum),na.rm=T) %>%
      tbl_zoomed(),
    dm_nycflights_small() %>%
      pull_tbl(airports) %>%
      select(3:6) %>%
  ...
  expect_equivalent_tbl(
    dm_nycflights_small() %>%
      dm_zoom_to(airports) %>%
      summarize_if(is_double,list(mean=mean,sum=sum),na.rm=T) %>%
      tbl_zoomed(),
    dm_nycflights_small() %>%  
      pull_tbl(airports) %>%
      summarize_if(is_double,list(mean=mean,sum=sum),na.rm=T)
...
test_that("output for compound keys",{
  ...
  expect_snapshot({
    ...
    zoomed_comp_dm %>%
      slice(c(1:3,5:3))
    zoomed_comp_dm %>%
      slice(c(1:3,5:3),.keep_pk=T) %>%
      col_tracker_zoomed()
    # FIXME: COMPOUND:: .keep_pk=F cannot deal with compound keys ATM

    # zoomed_comp_dm %>%  
    #   slice(c(1:3,5:3),.keep_pk=F) %>%
    #   get_tracked_cols()
    # JOINS
    # left_join()
    zoomed_comp_dm %>%
...
test_that("dm_keyed_tbl methods preserve keyed class",{
  ...
  expect_s3_class(reframe(group_by(tbl,e),d_mean=mean(d,na.rm=T)),"dm_keyed_tbl")
...
test_that(".by works with zoomed mutate()",{
  ...
  expect_equivalent_tbl(  
    dm_zoomed() %>%
      mutate(d_mean=mean(d,na.rm=T),.by=e) %>%
      tbl_zoomed(),
    tf_2() %>%
      mutate(d_mean=mean(d,na.rm=T),.by=e)
)
...
test_that(".by works with keyed mutate()",{
  ...
  dm<-dm_for_filter()
  tbl<-keyed_tbl_impl(dm,"tf_2")
  result<-mutate(tbl,d_mean=mean(d,na.rm=T),.by=e)
  expect_s3_class(result,"dm_keyed_tbl")
...
test_that(".by works with zoomed summarise()",{
  expect_equivalent_tbl(
    dm_zoomed() %>%
      summarise(d_mean=mean(d,na.rm=T),.by=e) %>%
      tbl_zoomed(),
    tf_2() %>%
      summarise(d_mean=mean(d,na.rm=T),.by=e)
...
test_that(".by works with keyed summarise()",{
  ...
  result<-summarise(tbl,d_mean=mean(d,na.rm=T),.by=e)
  expect_s3_class(result,"dm_keyed_tbl")
  expect_true("d_mean" %in% colnames(result))
...
test_that(".by works with zoomed reframe()",{
  expect_equivalent_tbl(
    dm_zoomed() %>%
      reframe(d_mean=mean(d,na.rm=T),.by=e) %>%
      tbl_zoomed(),
    tf_2() %>%
      reframe(d_mean=mean(d,na.rm=T),.by=e)
)
...
test_that("reframe returns any number of rows per group",{
  ...
  expect_equivalent_tbl(
    ...
    dm_zoomed() %>%  
      group_by(e) %>%
      reframe(d_vals=range(d,na.rm=T)) %>%
      tbl_zoomed(),
    tf_2() %>%
      group_by(e) %>%
      reframe(d_vals=range(d,na.rm=T))
