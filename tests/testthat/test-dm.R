test_that("dm() API",{
  expect_snapshot({
    ...
    dm(a=tibble(),a=tibble(),.name_repair="unique",.quiet=T)
})
...
test_that("dm() works for adding tables",{
  ...
  expect_identical(
    dm(dm_for_filter(),tf_1=data_card_1(),.name_repair="unique",.quiet=T) %>%
      src_tbls_impl(),
...
test_that("auto-renaming works",{
  ...
  expect_silent(
    dm(dm_for_filter(),dm_for_flatten(),dm_for_filter(),.name_repair="unique",.quiet=T)
)
})
...
test_that("'compute.dm()' fails with `temporary=F` (#2059)",{
  expect_snapshot(error=T,{
    dm_for_filter_duckdb() %>%
      compute(temporary=F)
})
...
test_that("`pull_tbl()`-methods work",{
  ...
  expect_equal(
    pull_tbl(dm_for_filter(),tf_5,keyed=T),  
    dm_get_tables(dm_for_filter(),keyed=T)[["tf_5"]]  
)
})

test_that("`pull_tbl()` keyed=T fails for zoomed dm",{
  expect_snapshot(error=T,{
    dm_for_filter() %>%
      dm_zoom_to(tf_1) %>%
      pull_tbl(keyed=T)
...
test_that("`pull_tbl()`-methods work (2)",{
  ...
  expect_dm_error(
    dm_for_filter() %>%
      ...
      dm_from_def(zoomed=T,validate=F) %>%
      pull_tbl(),
