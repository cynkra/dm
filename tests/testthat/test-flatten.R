  ...
  expect_snapshot(
{
  ...
  # deeper hierarchy available and `auto_detect=T`
...
test_that("`dm_flatten_to_tbl(.recursive=T)` does the right things",{
  # with grandparent table
  # left_join:
  expect_equivalent_tbl(
    dm_flatten_to_tbl(dm_more_complex(),tf_5,tf_4,tf_3,.recursive=T),
    tf_5() %>%
      left_join(tf_4(),by=c("l"="h")) %>%
      left_join(tf_3(),by=c("j"="f","j1"="f1"))
)
  ...
  expect_identical(
    ncol(dm_flatten_to_tbl(dm_more_complex(),tf_5,.recursive=T)),
    12L
)
  # semi_join:
  expect_dm_error(
    dm_flatten_to_tbl(dm_more_complex(),tf_5,tf_4,tf_3,.join=semi_join,.recursive=T),
    class="squash_limited"
)

  # anti_join:
  expect_dm_error(
    dm_flatten_to_tbl(dm_more_complex(),tf_5,tf_4,tf_3,.join=anti_join,.recursive=T),
    class="squash_limited"
)

  # fails when there is a cycle:  
  expect_dm_error(
    dm_flatten_to_tbl(dm_for_filter_w_cycle(),tf_5,.recursive=T),
    "no_cycles"
)
  ...
  # full_join:
  expect_equivalent_tbl(
    dm_flatten_to_tbl(dm_more_complex(),tf_5,tf_4,tf_3,.join=full_join,.recursive=T),
    tf_5() %>%
  ...
  # right_join:
  expect_dm_error(
    dm_flatten_to_tbl(dm_more_complex(),tf_5,tf_4,tf_3,.join=right_join,.recursive=T),
    class="squash_limited"
)
})
...
test_that("prepare_dm_for_flatten() works",{
  ...
  expect_equivalent_dm(
    out,
    dm_select_tbl(dm_for_flatten(),fact,dim_1,dim_3)%>%dm_disambiguate_cols(.quiet=T)
)

  # without rename
  expect_equivalent_dm(
    prepare_dm_for_flatten(dm_for_flatten(),c("fact","dim_1","dim_3"),gotta_rename=F),
