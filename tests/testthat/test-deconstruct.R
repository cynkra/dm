...
test_that("`new_keyed_tbl()` generates expected output",{
  ...
  expect_equal(dm$airports,keyed_tbl,ignore_attr=T)
})
...
test_that("`new_keyed_tbl()` formatting",{
  ...
  expect_snapshot({
    keyed_tbl_impl(dm_nycflights13(cycle=T),"flights")
    keyed_tbl_impl(dm_nycflights13(cycle=T),"airports")
    keyed_tbl_impl(dm_nycflights13(cycle=T),"airlines")
})
})
# subsetting ----------------------------------
...
test_that("summarize for keyed tables produces expected output",{
  ...
  # FIXME: Brittle tests?  
  local_options(dplyr.summarise.inform=F)
