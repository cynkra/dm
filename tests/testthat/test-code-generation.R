test_that("code generation works", {
  expect_identical(
    new_cg_block(),
    structure(
      list(cg_input_object = list(), cg_f_list = list()),
  ...
  expect_snapshot({
    ...
    new_cg_block(
      quo(dm_nycflights13()),
      list(function(.) dm_add_pk(., flights, flight_id))
    )
  })
  ...
  expect_snapshot({
    ...
    cg_block <- new_cg_block(quo(dm_nycflights13())) %>%
      ...
      cg_add_call(dm_rm_fk(
        .,
        table = flights,
        columns = c(origin, time_hour),
        ref_table = weather
      )) %>%
      cg_add_call(dm_add_fk(
        .,
        table = !!sym(table),
        columns = !!sym(columns),
        ref_table = airlines
