...
test_src_sqlite<-function() {
  ...
  dbplyr::src_dbi(DBI::dbConnect(RSQLite::SQLite(),":memory:"),auto_disconnect=T)
}
...
test_src_duckdb<-function() {
  ...
  dbplyr::src_dbi(DBI::dbConnect(duckdb::duckdb()),auto_disconnect=T)  
}
...
test_src_postgres<-function() {
  ...
  dbplyr::src_dbi(con,auto_disconnect=T)
}

test_src_maria<-function(root=F) {
...
test_src_mssql<-function(database=T) {
  ...
  DBI::dbExecute(con,"SET IMPLICIT_TRANSACTIONS OFF",immediate=T)
