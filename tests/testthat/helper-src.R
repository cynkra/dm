...
copy_to_my_test_src<-function(rhs,lhs) {
  ...
  if(is.null(src)) {
    ...
    suppressMessages(
      map(rhs,~ copy_to(src,.x,name=unique_db_table_name(name),temporary=T))
)
} else {
    suppressMessages(copy_to(src,rhs,name=name,temporary=T))
}
...
test_src_frame<-function(...,.temporary=T,.env=parent.frame(),.unique_indexes=NULL) {
  src<-my_test_src()
...
dm_for_filter_w_cycle %<-%
{
    dm(  
      ...
      dm_add_pk(tf_1,a,autoincrement=T) %>%
      #
