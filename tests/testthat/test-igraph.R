...
make_graph_abc<-function(directed=T) {
...
test_that("graph_from_data_frame: print output is consistent",{
  ...
  g_directed<-graph_from_data_frame(d,directed=T,vertices=c("a","b","c"))
  g_undirected<-graph_from_data_frame(d,directed=F,vertices=c("a","b","c"))
  expect_snapshot({
...
test_that("graph_from_data_frame: non-empty data frame builds correct graph",{
  d<-tibble::tibble(from=c("a","b"),to=c("b","c"))
  g<-graph_from_data_frame(d,directed=T,vertices=c("a","b","c"))
...
test_that("graph_from_data_frame: zero-row data frame builds graph with no edges",{
  ...
  g<-graph_from_data_frame(d,directed=F,vertices=c("x","y"))
...
test_that("graph_dfs visits all reachable vertices from root",{
  g<-make_graph_abc()
  dfs<-graph_dfs(g,"a",unreachable=F,dist=T)
  visited<-names(dfs$order)[!is.na(names(dfs$order))]
  expect_setequal(visited,c("a","b","c","d"))
})

test_that("graph_dfs dist=T gives 0 distance for root",{  
  g<-make_graph_abc()
  dfs<-graph_dfs(g,"a",unreachable=F,dist=T)
...
test_that("graph_dfs parent=T gives NA parent for root",{
  g<-make_graph_abc()
  dfs<-graph_dfs(g,"a",unreachable=F,parent=T)
...
test_that("graph_shortest_paths predecessors has one entry per vertex",{
  g<-make_graph_abc(directed=F)
  sp<-graph_shortest_paths(g,"a",names(graph_vertices(g)),predecessors=T)
...
test_that("graph_topo_sort via create_graph_from_dm: parent tables before children",{
  skip_if_not_installed("nycflights13")
  dm<-dm_nycflights13()
  g<-create_graph_from_dm(dm,directed=T)
  topo<-graph_topo_sort(g,mode="in")
