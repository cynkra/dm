...
make_dm_graph_abc<-function(directed=T) {
  new_dm_graph(
    directed=directed,
    vnames=c("a","b","c","d"),
    from=c(1L,2L,2L),
...
test_that("fallback graph_dfs matches igraph graph_dfs: visited vertices",{
  skip_if_not_installed("igraph")
  fb<-graph_dfs_fallback(make_dm_graph_abc(),"a",unreachable=F,dist=T)
  ig<-graph_dfs(make_graph_abc(),"a",unreachable=F,dist=T)
...
test_that("fallback graph_distances matches igraph graph_distances",{
  skip_if_not_installed("igraph")
  g_fb<-make_dm_graph_abc(directed=F)
  g_ig<-make_graph_abc(directed=F)
...
test_that("fallback graph_shortest_paths: predecessors count matches igraph",{  
  ...
  g_ig<-make_graph_abc(directed=F)
  all_v<-c("a","b","c","d")
  fb<-graph_shortest_paths_fallback(g_fb,"a",all_v,predecessors=T)
  ig<-graph_shortest_paths(g_ig,"a",all_v,predecessors=T)
