...
test_that("key tracking works",{
  ...
  expect_snapshot({
    ...
    dm_zoomed() %>%  
      ...
      dm_zoom_to(tf_2) %>%
      separate(new_col,c("c","e"),remove=T) %>%
      dm_update_zoomed() %>%
      get_all_keys()
    ...
    dm_zoomed() %>%
      ...
      dm_add_fk(tf_2,new_col,tf_6) %>%  
      dm_zoom_to(tf_2) %>%
      separate(new_col,c("c","e"),remove=F) %>%
      dm_update_zoomed() %>%
      get_all_keys()  
})
})
...
test_that("output for compound keys",{  
  ...
  expect_snapshot({
    ...
    nyc_comp() %>%
      dm_zoom_to(weather) %>%
      separate(origin,c("o1","o2"),sep="^..",remove=T) %>%
      dm_update_zoomed()

    nyc_comp() %>%  
      dm_zoom_to(weather) %>%
      separate(origin,c("o1","o2"),sep="^..",remove=F) %>%
      dm_update_zoomed()
})
})
