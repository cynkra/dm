...
test_that("table_description works",{
  ...
  expect_snapshot_diagram(  
    ...
    dm_nycflights13(table_description=T) %>%  
      dm_draw(
        backend_opts=list(font_size=c(table_description=6L,header=19L,column=14L))
  ),  
    "table-desc-4-dm.svg"
...
test_that("unsupported backend_opts fail with clear error",{
  expect_snapshot(
    dm_nycflights13() |> dm_draw(backend_opts=list(columnArrows=T)),
    error=T
)
...
test_that("column_arrow backend option works",{
  ...
  expect_no_warning(
    dm_nycflights13() |>
      dm_draw(backend_opts=list(column_arrow=F)) |>
      invisible()
)
})
