...
We can get the list of tables with `cdm_get_tables()` and the `src` object with `dm_get_src()`.
...
`cdm_add_pk()` has an option to check if the column of the table given by the user is a unique key; for performance reasons, the check will not be executed unless requested.
