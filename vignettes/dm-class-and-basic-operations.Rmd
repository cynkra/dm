...
data_model <- dm_get_data_model(iris_dm)
another_iris_dm <- 
  new_dm(
    tables = tables,
...
You saw, that with `dm_get_data_model()` we access the `data_model` part of a `dm`.
Similarly, we can get the list of tables with `dm_get_tables()` and the `src` object with `dm_get_src()`.
...
Get an overview over all tables with primary keys, use `dm_get_all_pks()`:

```{r}
dm_get_all_pks(dm_nycflights13(cycle = TRUE))
```

We used here the prepared `dm` object `dm_nycflights13(cycle = TRUE)` as an example. 
...
Get an overview over all foreign key relations in your data model with`dm_get_all_fks()`:
