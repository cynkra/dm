...
Another function for getting to know your data better (cf. `cdm_enum_pk_candidates()` in ["Primary keys of `dm` objects"](#pk)) is `dm_enum_fk_candidates()`. Use it to get an overview over foreign key candidates that point from one table to another:
...
```{r}
dm_get_all_fks(cdm_nycflights13(cycle = TRUE))
```
