...
Although the example that we will be using is included in `dm`, and you can see it by running `cdm_nycflights13()`, we will be going through the built-in example one more time here, step by step.
...
To review the primary keys after setting them, call `cdm_get_all_pks()`.
...
To define how your tables are related, use `cdm_add_fk()` to add foreign keys. First, define the tables that you wish to connect by parameterizing the `cdm_add_fk()` function with `table` and `ref_table` options. 
