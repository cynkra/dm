...
Further Reading: The {dm} methods for [visualizing data models](https://krlmlr.github.io/dm/articles/tech-dm-draw.html).
...
A primary key can be either an existing column that satisfies the condition of being unique, or a new column that assigns an identifier.
...
Further Reading: The {dm} package offers several functions for dealing with [primary keys](https://krlmlr.github.io/dm/articles/tech-dm-class.html#pk).
...
Further Reading: All {dm} functions for working with [foreign keys](https://krlmlr.github.io/dm/articles/tech-dm-class.html#foreign-keys).
...
If a foreign key contains a parent table primary key that has been deleted that row is an orphan row and the database no longer has referential integrity.

{dm} is designed to maintain referential integrity.
It will not let you create a child row or table without first creating the parent.
For example, in the `nycflights13` you can't create a flight for an airline that does not have an entry in the `airlines` table.
Conversely, you cannot delete the row for an airline from the `airlines` table until all related flights are deleted.
For more information on deleting individual rows and other row-level operations see `vignette("howto-dm-rows")`.
