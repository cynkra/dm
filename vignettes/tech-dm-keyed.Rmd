...
The `dm_get_tables()` and `pull_tbl()` functions have a new experimental argument `keyed`, which defaults to `F`.
If set to `T`, a list of objects of class `dm_keyed_tbl` is returned instead.
...
If this workflow proves as useful as it seems, subsetting tables via `$`, `[[` will default to `keyed=T` in a forthcoming major release of {dm}.
...
flights_dm<-dm_nycflights13(cycle=T)
flights_dm
flights_keyed <-
  flights_dm %>%
...
If you look at the `dm` created by `dm_nycflights13(cycle=T)`, you see that two columns of `flights` relate to the same table, `airports`.
