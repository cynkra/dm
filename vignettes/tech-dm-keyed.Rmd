...
This vignette deals with situations where you want to transform tables of your `dm` object
...
These objects will also attempt to track primary and foreign keys,
so that they are available for joins and when recombining these tables later into a `dm` object.
...
Reconstructing the `dm` object is not strictly necessary
if you're primarily interested in deriving one or multiple separate tables for analysis.

If this workflow proves as useful as it seems, subsetting tables via `$`,
`[[` will default to `keyed = TRUE` in a forthcoming major release of {dm}.
...
you see that two columns of `flights` relate to the same table, `airports`.
...
such as `dm_flatten_to_tbl()`, `dm_filter()` or `dm_wrap_tbl()`.
...
you can make use of one of the `..._join()` methods for a `dm_keyed_tbl`.
In many cases, using keyed tables derived from a `dm` object allows omitting the `by` argument without triggering a message,
because they are safely inferred from the foreign keys stored in the `dm_keyed_tbl` objects.
