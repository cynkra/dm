...
1. individually subsetting the tables relevant to the calculation, perform the necessary transformations, and recombine the resulting table into a `dm`.
1. do all this within the `dm` object by zooming to a table and manipulating it.

Both approachas aim at maintaining the key relations whenever possible.
...
The `dm_get_tables()` function has a new experimental argument `keyed`, which defaults to `FALSE`.
If set to `TRUE`, a list of objects of class `dm_keyed_tbl` is returned instead.
Because `dm_keyed_tbl` inherits from `tbl` or `tbl_lazy`, many {dplyr} and {tidyr} will work unchanged.
However, these objects will attempt to track primary and foreign keys, so that they are available for joins and when recombining these tables later into a `dm` object.
When you are finished with transforming your data, use `dm()` or `new_dm()` to recombine the tables into a `dm` object.
...
If this workflow proves as useful as it seems, subsetting tables via `$`, `[[` and `pull_tbl()` will default to `keyed = TRUE` in a forthcoming major release of dplyr.
...
derived_flights_dm <- dm(dep_carrier_count, !!!flights_keyed)
...
The left-hand-side table of a join is always the subsetted table.
