...
vignette: >
  %\VignetteIndexEntry{Technical: Manipulating tables}
...
<!-- ### Tip: Accessing the subsetted table -->

<!-- At each point, you can retrieve the subsetted table by calling `pull_tbl()` on a `dm_keyed_tbl`. -->
...
<!-- 1. Currently, not all {dplyr}-verbs have their own method for a `dm_keyed_tbl` object, so be aware that in some cases it will still be necessary to resort to extracting one or more tables from a `dm` and reinserting a transformed version back into the `dm` object. -->
