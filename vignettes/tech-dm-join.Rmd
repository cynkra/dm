...
This document introduces you to the joining functions of {dm}
and shows how to apply them using data from the [{nycflights13}](https://github.com/tidyverse/nycflights13) package.

[Relational data models](https://dm.cynkra.com/articles/howto-dm-theory#model) consist of multiple tables
that are linked with [foreign keys](https://dm.cynkra.com/articles/howto-dm-theory#fk).
They are the building blocks for joining tables.
...
with 3,322 different planes and 1,458 airports involved.
...
{dm} allows you to join two tables of a `dm` object based on a shared column.
You can use all join functions that you know from the {dplyr} package.
...
In technical terms, we merge the tables
that need to be directly connected by a [foreign key relation](https://dm.cynkra.com/articles/howto-dm-theory#fk).
...
**Get all May 2013 flights from Delta Air Lines which didn't depart from John F. Kennedy International Airport in -
and join all the airports data into the `flights` table.**
...
The `dm_flatten_to_tbl()` takes care of this by automatically renaming the relevant columns and informs the user if any names were changed,
