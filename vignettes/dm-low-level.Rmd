...
To see if a column of one table contains only values, that are also present in another column of another table (one of the two criteria of it being a foreign key to this other table, the other being if this other table's column is one of its unique keys), you can use `check_if_subset()`:
...
`0_n` means, that for each value of the `parent_key_column`, minimally '0' and maximally 'n' values have to correspond to it in the child table's column (which translates to no further restriction).
...
## Examples for functions of the 'table_surgery' family
...
It might sometimes be helpful, to be able to split a table in a way, that a parent table (i.e. a lookup table) and a child table (containing the observations) are created, including a column (here: `new_id_column`) for mapping the child table observations to the parent table lookup rows.
