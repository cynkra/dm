...
In this document we will present several specialised functions for conducting basic tests about key conditions and about relations between tables.
...
1. `examine_key(.data, ...)`
1. `examine_if_subset(t1, c1, t2, c2)`
1. `examine_set_equality(t1, c1, t2, c2)`
...
and you want to know if `a` is a primary key for `data_1`, you can use the `examine_key()` function:

```{r}
examine_key(data_1, a)
```
...
```{r}
examine_key(data_2, a)
```

To see if a column of one table contains only those values that are also present in another column of another table, the `examine_if_subset()` function can be used:
...
```{r}
examine_if_subset(data_2, a, data_1, a)
```

It should be kept in mind that `examine_if_subset()` does not test if column `c2` is a unique key of table `t2`.
...
```{r, eval = FALSE}
examine_key(t2, c2)
examine_if_subset(t1, c1, t2, c2)
```

To check both directions at once, and to find out if the unique values of `c_1` in `t_1` are the same as those of `c_2` in `t_2`, {dm} provides the function `examine_set_equality()` (this would fail with a longer error message and is therefore not evaluated):

```{r eval=FALSE}
examine_set_equality(data_1, a, data_2, a)
```
...
1. `examine_cardinality_0_n(parent_table, primary_key_column, child_table, foreign_key_column)`
1. `examine_cardinality_1_n(parent_table, primary_key_column, child_table, foreign_key_column)`
1. `examine_cardinality_0_1(parent_table, primary_key_column, child_table, foreign_key_column)`
1. `examine_cardinality_1_1(parent_table, primary_key_column, child_table, foreign_key_column)`
...
examine_cardinality_0_n(d2, c, d1, a)
...
examine_cardinality_1_1(d1, a, d2, c)
