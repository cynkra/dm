...
1. `check_cardinality(parent_table, primary_key_column, child_table, foreign_key_column)`
...
Also `check_cardinality()` first performs the above mentioned tests to figure out, if the parent-child table relationship criteria are met.
...
`check_cardinality()` returns the type of relation, e.g.:
```{r}
check_cardinality(d1, a, d3, c)
check_cardinality(d1, a, d2, c)
check_cardinality(d1, a, d1, a)
check_cardinality(d1, a, d4, a)
```
