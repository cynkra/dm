...
To see if a column of one table contains only those values that are also present in another column of another table, the `check_if_subset()` function can be used:
...
It should be kept in mind that `check_if_subset()` does not test if column `c2` is a unique key of table `t2`.
...
To check both directions at once, and to find out if the unique values of `c_1` in `t_1` are the same as those of `c_2` in `t_2`, {dm} provides the function `check_set_equality()` (this would fail with a longer error message and is therefore not evaluated):
