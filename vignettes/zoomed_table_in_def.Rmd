...
We need to decide if we want an extra column in `def` for the filter condition of the zoomed table.
In this vignette we currently don't have an extra column, the condition is written into the filter column of the table that the zoomed table is based on.
This would also imply that `cdm_apply_filters()` and the methods for `tbl()`, `[[.dm()`, `$.dm()` should not be allowed to work with a `zoomed_dm`.
...
FKs pointing to the old table are NOT pointing to the new one.
...
(new_def <- cdm_add_tbl(cdm_nycflights13(), flights_new) %>%
  ...
  mutate(
    ...
    fks = if_else(
      table == "planes",
      vctrs::list_of(dm:::new_fk(table = c("flights_new", "flights"), column = list("plane_code", "tailnum"))),
      fks)
    ) %>% 
  left_join(key_tracker_zoom, by = "table")
)
