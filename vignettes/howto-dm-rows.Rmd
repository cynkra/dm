...
Be aware that when using `dm_examine_constraints()` foreign keys may be NULL (`NA`).
Null foreign keys are counted as a match. 
...
dm objects whose source is a database treat all operations as temporary unless you state explicitly by including the argument `temporary=FALSE` to commit them. 
...
To demonstrate `dm_rows_insert()` we create a dm with tables containing the rows we want to insert and copy it to `sqlite_db`, the same source as `demo_sql`.
...
This gives us a warning that changes will not be persisted, and inspecting the `child` table of the resulting `dm_d` and `demo_sql`, we can see that's exactly what happened.
...
# Truncate
dm_truncated <- dm(parent=tibble(value='D', pk=4)) %>%
                dm_rows_truncate(dm_upserted, .)
dm_truncated$parent
dm_truncated$child
