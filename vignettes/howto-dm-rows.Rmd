...
``````{r }
dm_insert_in <-
  dm(parent = tibble(value = "D", pk = 4), child = tibble(value = "d", pk = 4, fk = 4)) %>%
...
dm_update_in <-
  dm(child = tibble(value = "b", pk = 2, fk = 2)) %>%
  copy_dm_to(sqlite_db, ., temporary = TRUE)
...
``````{r }
dm_patched <-
  dm(child = tibble(value = "c", pk = 3, fk = 3)) %>%
...
``````{r }
dm_upserted <-
  dm(parent = tibble(value = "D", pk = 4), child = tibble(value = c("b", "d"), pk = c(2, 4), fk = c(3, 4))) %>%
