...
1. If desired, simulate changes with `in_place=F` to double-check

1. Apply changes with `in_place=T`.
...
With the dm in the same RDBMS as the destination dm, you call the appropriate method, such as `dm_rows_insert()`, to make your planned changes, along with an argument of `in_place=F` so you can confirm you achieve the changes that you want.
...
With the changes confirmed, you execute the method again, this time with the argument `in_place=T` to make the changes permanent.
Note that `in_place=F` is the default: you must opt in to actually change data on the database.
...
As `demo_sql` will be the destination dm for the examples, the argument `temporary=F` is used to make this distinction apparent.
...
demo_sql<-copy_dm_to(sqlite_db, demo_dm, temporary=F)
demo_sql
``````
...
dm_insert_in <-
  ...
  copy_dm_to(sqlite_db, ., temporary=T)
``````
...
We repeat the operation, this time with the argument `in_place=T` and the changes now persist in `demo_sql`.

``````{r }
dm_insert_out <-
  demo_sql %>%
  dm_rows_insert(dm_insert_in, in_place=T)
...
dm_update_out <-
  ...
  dm_rows_update(dm_update_in, in_place=T)
