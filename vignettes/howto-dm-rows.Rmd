...
Foreign keys in `City` link rows to a row in `Province` and foreign keys in `Province` link rows to a row in `Country`.
If it was discovered that Molvania, an imaginary country, was accidentally included in the database, on providing a dm object containing the `Country`, `Province` and `City` tables with all rows associated with it, the entries in the `City` table would be deleted first, followed by entries in the `Province` table, then its row in the `Country` table.
...
``````{r }
dm_insert_in <- dm(parent=tibble(value='D',pk=4),child=tibble(value='d',pk=4,fk=4)) %>%
              copy_dm_to(sqlite_db, ., temporary=TRUE)
dm_insert_out <- demo_sql %>% dm_rows_insert(dm_insert_in)
dm_insert_out$child
demo_sql$child
...
``````{r }
dm_insert_out <- demo_sql %>% dm_rows_insert(dm_insert_in, in_place=TRUE)
demo_sql$child
...
dm_update_in <- dm(child=tibble(value='b',pk=2,fk=2)) %>% 
              copy_dm_to(sqlite_db, ., temporary=TRUE)

dm_update_out <- demo_sql %>% dm_rows_update(dm_update_in, in_place=TRUE )
demo_sql$child
...
To demonstrate `dm_rows_truncate()` we can create a dm object with a tibble containing the values for a single row to specify the table we want to truncate.

``````{r }

dm_trunc_in <- dm(child=tibble(value='b',pk=2,fk=2)) %>%
            copy_dm_to(sqlite_db, ., temporary=TRUE)
dm_trunc_out  <- demo_sql %>% dm_rows_truncate(dm_trunc_in,in_place=FALSE)
dm_trunc_out$child

``````

We can also start with a pre-existing dm, `demo_dm`, and remove the tables we don't want to truncate.

``````{r }

dm_trunc_in <- demo_dm %>% dm_rm_tbl(parent) %>%
            copy_dm_to(sqlite_db, ., temporary=TRUE)
dm_trunc_out  <- demo_sql %>% dm_rows_truncate(dm_trunc_in,in_place=TRUE)
dm_trunc_out$child
dm_trunc_out$parent
