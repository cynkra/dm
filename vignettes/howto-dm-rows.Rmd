...
1. Create a temporary *changeset dm* object that defines the intended changes on the RDBMS <!-- The technical term "changeset" needs to be explained here -->
1. If desired, simulate changes with `in_place = FALSE` to double-check
1. Apply changes with `in_place = TRUE`.

<!-- TODO: the following explanation is difficult to follow in prose, 
brainstorm if we can come up with a visual illustration instead -->
