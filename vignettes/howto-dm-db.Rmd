...
{dm} was designed to make connection to and working with an RDBMS as straightforward as possible.
...
To demonstrate, we will connect to a [relational dataset repository](https://relational.fit.cvut.cz/) with a database server that is publicly accessible without registration.
It hosts a [financial dataset](https://relational.fit.cvut.cz/dataset/Financial) that contains loan data along with relevant account information and transactions.
...
Operations on dm objects are limited to report only the first ten results.
`collect()` forces the evaluation of all SQL queries and the generation of the complete set of results.
The resulting tables are transferred from the RDBMS and returned as local tibbles.
...
dm implments many of the {dplyr} methods for making changes you have made to the database permanent.
For example, `compute()` forces the computation of the queries created by a dm object to represent operations that have been performed but not yet evaluated.
The results of `compute()` are stored in materialized tables on the database server.
...
It is just as simple to move a local relational model into an RDBMS as it using `collect()` to download it.
...
`copy_dm_to()` and `compute()` are covered in more detail in `vignette("howto-dm-copy")`.
