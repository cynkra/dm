...
For this, we use `learn_keys=F` to obtain a `dm` object with only the tables.

``````{r load}
library(dm)


fin_dm<-dm_from_con(my_db, learn_keys=F)
...
The columns in the temporary table are constrained to the columns passed as the `.by` argument and the column(s) created by the `summarize()` function.
