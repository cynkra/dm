...
If set to `T`, the function checks if the column of the table given by the user is unique.
For performance reasons, the default is `check=F`.
See also [dm_examine_constraints()] for checking all constraints in a `dm`.


```{r error=T}
try(
  dm_add_pk(flights_dm, airports, tzone, check=T)
)
```
...
```{r error=T}
try(
  flights_dm_with_fk%>%dm_add_fk(flights, dest, airports, check=T)
)
```

Checks are opt-in and executed only if `check=T`.
You can still add a foreign key with the default `check=F`.
...
```{r}
dm_get_all_fks(dm_nycflights13(cycle=T))
```
