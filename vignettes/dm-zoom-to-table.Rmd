...
When employing one of the first two options, the resulting table in the `dm` will have all the primary and foregin keys available that could be tracked from the originally zoomed table.
...
In addition to the usual arguments for the {dplyr}-joins, by supplying an argument for `select` you can specify which columns of the RHS-table you want to be included in the join.
