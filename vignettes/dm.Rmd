...
```
Now we create and store an object with two foreign key relations (both pointing from table `flights` to table `airports`):
```{r}
flights_dm_with_fk <- 
  cdm_add_fk(flights_dm, flights, origin, airports, faa, set_ref_pk = TRUE) %>% 
  cdm_add_fk(flights, dest, airports, faa)
```
