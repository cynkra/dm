...
These components can be accessed with `dm_get_src()` and `dm_get_tables()`.

```{r}
dm_get_src(flights_dm)
dm_get_filter(flights_dm)
...
1. If you access a table via `cdm_apply_filters_to_tbl()`, a sequence of `semi_join()` calls is performed to retrieve the requested table with only those values in the key columns which correspond to the remaining values in the filtered tables

The function `cdm_apply_filters()` essentially calls `cdm_apply_filters_to_tbl()` for each table of the `dm` and creates a new `dm` object from the result.
...
```{r}
cdm_apply_filters_to_tbl(us_flights_from_jfk, "planes")
```
