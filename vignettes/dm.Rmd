...
3. the meta-data from the data model: the key constraints
...
1. [How to create a `dm` object](#create)
2. [Setting primary and foreign keys](#keys)
3. [Graphically represent a `dm` object](#vis)
4. [Filtering a `dm` object](#filter)
5. [Transfer to and learning from a DB](#copy)
...
## Handling keys of `dm` objects {#keys}
...
## Visualizing the data model {#vis}
...
The relevant verb is `cdm_copy_to()`.
...
Here we show an example with in-memory SQLite-DB (so no key constraints are established):
...
Unfortunately, it is not possible to execute the corresponding code in this vignette, since -- just like with the transfer to a DB -- only MSSQL and Postgres are so far supported, and there are no generally accessible DBs of this type available.
