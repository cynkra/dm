...
Additionally, most `dm` functions are pipe-friendly and support tidy evaluation.
...
In our data model, `id` columns uniquely identify records in the `accounts` and `loans` table, and can be used as a primary key. A primary key is added with `dm_add_pk()`.
