...
1. the meta-data from the data model: the key constraints
1. the tables themselves, i.e., the data
...
1. FIXME: `cdm_nycflights13() %>% cdm_join_tbl(airlines, flights, join = left_join)`
1. [Filtering](#filter)
...
FIXME: `cdm_get_xxx()`.
Individual tables can be accessed with `tbl()`.
...
Our helper has an option for this:
