...
1. `dm_add_key()`
2. `dm_check_if_table_has_pk()`
3. `dm_get_pk_column_from_table()`
4. `dm_remove_pk()`
...
So let's add one.
By default `dm_add_key()` checks if the column of the table given by the user is a unique key.
Since the `iris` dataset does not have any unique one-column keys, we will use the `nycflights13` tables, i.e. `flights_dm`:

```{r}
dm_check_if_table_has_pk(flights_dm, "airports")
```
```{r}
...
```{r}
dm_check_if_table_has_pk(flights_dm_with_key, "airports")
```
Get the name of the column that is marked as primary key of the table:
```{r}
dm_get_pk_column_from_table(flights_dm_with_key, "airports")
```
Remove a primary key by:
```{r}
dm_remove_pk(flights_dm_with_key, "airports") %>% 
  dm_check_if_table_has_pk("airports")
```
