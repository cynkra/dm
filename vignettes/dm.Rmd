...
(If your data is in data frames instead of a database and you're in a hurry, jump over to `vignette("dm-df")`.)
...
Here we focus on the `loans`, `accounts`, `districts` and `trans` tables:

```{r select}
fin_dm_small <- fin_dm[c("loans", "accounts", "districts", "trans")]
```

The `dm_select_tbl()` verb offers a way to do the same in a `%>%` pipe.
...
If you want to make permanent changes to your data model on an RDBMS please see the "Persisting results" section in `vignette("dm-db")`.
...
For more on constraint checking, including cardinality, finding candidate columns for keys, and normalization, see `vignette("dm-low-level")`.
...
`vignette("dm-df")` - Is your data in local data frames? This article covers creating a data model from your local data frames, including building the relationships in your data model, verifying your model, and leveraging the power of dplyr to operate on your data model.
...
`vignette("dm-filtering")` - Need a subset of your data across tables? This introduction to dm's filtering methods will show you how to apply them to data that is separated into multiple tables.
...
`vignette("dm-class-and-basic-operations")` - A detailed overview of the `"dm"` class.

`vignette("dm-function-naming-logic")` - Making sense of dm's API.

`vignette("dm-version-migration-guide")` - If you're an early adopter, used dm version 0.0.5 or lower and want to migrate your code, this is for you.
