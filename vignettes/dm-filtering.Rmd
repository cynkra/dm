...
To show the SQL query behind a `cdm_filter()`, we copy the `flights`, `airlines` and `airports` tables from the `nyflights13` dataset to a temporary in-memory database using the built-in function `dm_copy_to()` and `dbplyr::src_memdb`.
