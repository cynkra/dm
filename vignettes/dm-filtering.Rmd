...
1. Calling `cdm_apply_filters_to_tbl()`, i.e. requesting a table from a `dm` after the filter conditions are applied.
...
Each table is the result of a `cdm_apply_filters_to_tbl()` call, meaning that the effects of the filter conditions on each of the tables are taken into account.
...
Keep in mind that several {dm} functions will refuse to work when unevaluated filter conditions exist, such as, e.g. `cdm_enum_fk_candidates()`, `cdm_enum_pk_candidates()`, `cdm_select_tbl()`, `cdm_rename_tbl()`, `cdm_select()`, `cdm_rename()` and `dm_nrow()`.
...
The total number of rows in the `dm` drops from `r format(sum(dm_nrow(dm)), big.mark = " ")` to `r format(sum(dm_nrow(cdm_apply_filters(filtered_dm))), big.mark = " ")` (the only unaffected table is the disconnected `weather` table).
...
You can inspect the filtered tables with `cdm_apply_filters_to_tbl()`.

In the `airlines` table, Delta is the only remaining carrier:

```{r}
dm_delta_may %>% 
  cdm_apply_filters_to_tbl("airlines")
```
...
```{r}
dm_delta_may %>% 
  cdm_apply_filters_to_tbl("flights")
```
