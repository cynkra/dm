...
To show the SQL query behind a `dm_filter()`, we copy the `flights`, `airlines` and `airports` tables from the `nyflights13` dataset to a temporary in-memory database using the {dm}-method `copy_to()` for `src_sql` objects with the argument `dbplyr::src_memdb`.
...
dm %>%
  ...
  dm_filter(flights, month == 5) %>% 
  copy_to(dbplyr::src_memdb(), ., unique_table_names = TRUE) %>% 
  dm_filter(airlines, name == "Delta Air Lines Inc.") %>%
  dm_filter(airports, name != "John F Kennedy Intl") %>%
  dm_apply_filters() %>% 
  dm_get_tables() %>%
