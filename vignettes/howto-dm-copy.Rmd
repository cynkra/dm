...
deployed_dm<-copy_dm_to(local_db, fin_dm, temporary=F)
``````
...
my_dm_total <-
  deployed_dm %>%
  dm_zoom_to(loans) %>%
  summarize(.by=account_id, total_amount=sum(amount, na.rm=T)) %>%
...
stopifnot(grepl(remote_name_total_loans, sql_render(my_dm_total_computed$total_loans), fixed=T))
```
...
If you want results to persist across sessions in **permanent** tables, `compute()` must be called with the argument `temporary=F` and a table name for the `name` argument.
...
loans_df <-
  deployed_dm %>%
  dm_flatten_to_tbl(loans, .recursive=T) %>%
  select(id, amount, duration, A3) %>%
  collect()
``````

Please note the use of `recursive=T` for `dm_flatten_to_tbl()`.
...
The call to the method includes the argument `temporary=F` because we want this table to persist beyond our current session.
...
``````{r}
my_dm_sqlite_resid <-
  copy_to(deployed_dm, loans_residuals, temporary=F) %>%
...
deployed_dm <-
  copy_dm_to(destination_db, fin_dm, temporary=F)

deployed_dm
``````

Note that in the call to `copy_dm_to()` the argument `temporary=F` is supplied.
Without this argument, the model would still be copied into the database, but the argument would default to `temporary=T` and the data would be deleted once your session ends.
...
dup_dm <-
  ...
  copy_dm_to(destination_db, fin_dm, temporary=F, table_names=~ paste0("dup_", .x))
...
For tables that are created from a relational model during analysis or development, `compute()` and `copy_to()` can be used to persist them (using argument `temporary=F`) between sessions or to copy local tables to a database `dm`.
