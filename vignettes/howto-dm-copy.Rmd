...
The steps for this will be discussed in more detail in the [Copying a relational model](#copy-model) section.
...
The example below computes the total amount of all loans for each account.
...
The table name, `` `r dbplyr::remote_name(my_dm_total_computed$total_loans)` ``, was automatically generated as the `name` argument was not supplied.
...
It uses the {dm} convenience function `dm_financial()` (introduced in the previous tutorial, `vignette("howto-dm")`) to create a dm object corresponding to a data model from a public dataset repository. 
The dm object is downloaded locally first, before deploying it to a local SQLite database.

Before actually downloading any data, we check the number of rows with the `dm_nrow()` function:
