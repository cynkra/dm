...
In technical terms, we merge the tables that need to be directly connected by a [foreign key relation](https://krlmlr.github.io/dm/articles/dm-introduction-relational-data-models#fk).
...
2. Printed to the console by calling `cdm_get_all_fks()`
...
The `cdm_flatten_to_tbl` cares about automatically renaming the relevant columns and prints if something was changed, e.g. `name -> airlines.name`.
