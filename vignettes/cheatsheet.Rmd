...
create_figure <- function(class, src, alt, caption) {
  ...
  svg <- xml2::read_xml(src)
  xml2::xml_attr(svg, "aria-labelledby") <- sprintf("svgDescription-%s", src)
  xml2::xml_add_child(
    svg,
    .value = "description",
    alt,
    id = sprintf("svgDescription-%s", src),
    .where = 0
  )
  cat(as.character(svg))
  html <- sprintf('<p>%s</p>', commonmark::markdown_html(caption))
  cat(html)
}
```
...
```{r, results='asis'}
cat('<figure class="left">
  <img
    alt="Equal sign then a dm object where the blue table has now more rows."
    src="../man/figures/cheatsheet/insert2.svg"
  />
  <img
    alt="On the left a dm object with 3 tables. On the right, a similar dm where one of the tables, the blue one, has more rows. Between the two is a plus sign"
    src="../man/figures/cheatsheet/insert1.svg"
  />
</figure>')
```
...
cat('<figure class="left">
  <img
    alt="Database icon."
    src="../man/figures/cheatsheet/insert4.svg"
  />
  <img
    alt="On the left a dm object with 3 tables, and a database icon superimposed. On the right, a similar dm where one of the tables, the blue one, has more rows. Between the two is a plus sign"
    src="../man/figures/cheatsheet/insert3.svg"
  />
</figure>')
```
