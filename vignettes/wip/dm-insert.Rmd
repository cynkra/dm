...
rows_insert.data.frame <- function(.data, .key = NULL, ..., .sort = NULL) {
  ...
  stopifnot(!anyDuplicated(vec_rbind(.data[.key], source[.key])))
  out <- vec_rbind(.data, source)
  if (isTRUE(.sort)) {
    # FIXME: Faster implementation, sort before binding
    out <- arrange(out, !!!syms(.key))
  }
  out
}
...
dots_to_df <- function(.data, .key, ...) {
  ...
  stopifnot(all(.key %in% intersect(names(source), names(.data))))
...
rows_update.data.frame <- function(.data, .key = NULL, ...) {
  ...
  idx <- vec_match(source[.key], .data[.key])
...
```{r}
#' @export
rows_insert.tbl_dbi <- function(.data, .key = NULL, ..., 
                                .persist = NULL,
...
#' @export
rows_update.tbl_dbi <- function(.data, .key = NULL, ..., 
                                .persist = NULL,
                                .copy = NULL, .check = NULL) {
  .persist <- validate_persist(.persist)
  ...
  if (.persist) {
    ...
    # Checking optional, can rely on primary key constraint
    if (is_true(.check)) {
      check_superset(.data, source, .key)
    }
    
    unchanged <- anti_join(.data, source, by = .key)
    updated <- 
      .data %>% 
      select(!!!.key -!!!setdiff(colnames(.data), colnames(source))) %>% 
