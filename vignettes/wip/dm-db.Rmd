...
The repository uses a MariaDB server for which we do not currently import primary or foreign keys, so we will need to add them.
...
To force a dm to execute the SQL query generated by the operations it has performed, we call the `dbplyr::compute()` method on the dm object.
...
dm_financial_sqlite <- function() {
  ...
  my_dm <-
    ...
    dm_financial_sqlite() %>%
    # we remove the trans table from the dm due to its size
    dm_select_tbl(-trans)
...
# FIXME - short example showing a table added to sqlite_dm? 
