...
This is useful for when you want to work with a subset of a database's tables or you want to build a database out of tables from different sources.
...
A primary key is a column (or sometimes a combination of columns) that has a unique value for each row within a table.
A foreign key is a column (or sometimes a combination of columns) containing the primary key for a row in another table.
...
For tables from other sources or the local environment `[copy_to()](missing_link)` from the {dplyr} package is used.
...
As a workaround to demonstrate the usage of `compute()`, we will copy our dm into a local SQLite database and move forward with that as the source of our dm.
