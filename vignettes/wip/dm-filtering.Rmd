...
To explore filtering with `dm`, we'll use the `nycflights13` data with its tables `flights`, `planes` and `airports`.

This dataset contains all 336776 flights that departed from New York City in 2013 with 3322 different planes and 1458 airports involved.

The data comes from the US Bureau of Transportation Statistics, and is documented in `?nycflights13`.

First of all, we have to create a `dm` object from the `nycflights13` data. This is implmeneted with `dm::cdm_nycflights13`.
...
A `dm` object output with data and metadata consists of three elements: the table sources, the data model and a short glimpse into the data via the number of rows.
...
The total number of rows drops from  367687 to 138786 (apart from the disconnected `weather` table).

You can see this in the "Rows"-part of the print output.

```{r}
dm %>% cdm_filter(airports, name == "John F Kennedy Intl")
```
...
```{r}
dm %>% cdm_filter(flights, dest == "IAD")
```
...
In the table `airlines` American Airlines is the only remaining carrier:
