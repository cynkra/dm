...
Although {dm} is built upon relational data models, it is not a database itself. It can
work transparently with both relational database systems and in-memory data, and copy data [from and to databases](https://krlmlr.github.io/dm/articles/dm.html#copy).
...
The example data in {nycflights13} is integrated in `dm` and comes with preset primary and foreign keys to identify the common points between the tables.
For data other than the example data, the `dm` object would need to be created by using the `dm()` constructor.
...
For the package to work correctly, it is not enough to just clone or download the package, you will need to use a package manager in your R session to configure the necessary dependencies and to bind file locations to your local environment:
