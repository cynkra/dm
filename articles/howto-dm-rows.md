...
[`dm_rows_insert()`](https://dm.cynkra.com/reference/rows-dm.md), to
...
| Method                                                           | Requirements                                                                                                                                  |
|------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------|
| [`dm_rows_insert()`](https://dm.cynkra.com/reference/rows-dm.md) | Records with existing primary keys are silently ignored (via `dplyr::rows_insert(conflict = "ignore")`).                                      |
| [`dm_rows_append()`](https://dm.cynkra.com/reference/rows-dm.md) | All records are inserted, the underlying database might check for uniqueness of primary keys (and fail the operation) if a constraint is set. |
| [`dm_rows_update()`](https://dm.cynkra.com/reference/rows-dm.md) | Primary keys must match for all records to be updated.                                                                                        |
| [`dm_rows_patch()`](https://dm.cynkra.com/reference/rows-dm.md)  | Updates missing values in existing records. Primary keys must match for all records to be patched.                                            |
| [`dm_rows_upsert()`](https://dm.cynkra.com/reference/rows-dm.md) | Updates existing records and adds new records, based on the primary key.                                                                      |
| [`dm_rows_delete()`](https://dm.cynkra.com/reference/rows-dm.md) | Removes matching records based on the primary key. Primary keys must match for all records to be deleted.                                     |
...
keys). For
[`dm_rows_delete()`](https://dm.cynkra.com/reference/rows-dm.md), child
tables are processed before their parent tables. Note that the user is
still responsible for setting transactions to ensure integrity of
operations across multiple tables. For more details on this see
...
the method used,
[`copy_dm_to()`](https://dm.cynkra.com/reference/copy_dm_to.md), is to
create temporary tables that will be automatically deleted when your
...
To demonstrate
[`dm_rows_insert()`](https://dm.cynkra.com/reference/rows-dm.md), we
create a dm with tables containing the rows to insert and copy it to
`sqlite_db`, the same source as `demo_sql`. For all of the
[`dm_rows_...()`](https://dm.cynkra.com/reference/rows-dm.md) methods,
the source and destination `dm` objects must be in the same RDBMS. You
will get an error message if this is not the case.
...
[`dm_rows_update()`](https://dm.cynkra.com/reference/rows-dm.md) works
the same as
[`dm_rows_insert()`](https://dm.cynkra.com/reference/rows-dm.md). We
...
[`dm_rows_delete()`](https://dm.cynkra.com/reference/rows-dm.md) is not
currently implemented to work with an RDBMS, so we will shift our
...
[`dm_rows_patch()`](https://dm.cynkra.com/reference/rows-dm.md) updates
missing values in existing records. We use it here to fix the missing
...
[`dm_rows_upsert()`](https://dm.cynkra.com/reference/rows-dm.md) updates
rows with supplied values if they exist or inserts the supplied values
as new rows if they don’t. In this example we add the letter “D” back to
our dm, and update the foreign key for “b”.
...
The [`dm_rows_...()`](https://dm.cynkra.com/reference/rows-dm.md)
