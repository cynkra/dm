...
the [`check_key()`](https://dm.cynkra.com/reference/check_key.md)
function:
...
[`check_subset()`](https://dm.cynkra.com/reference/check_subset.md) does
not test if column `c2` is a unique key of table `t2`. In order to find
out if a (child) table `t1` contains a column `c1` that is a foreign key
to a (parent) table `t2` with the corresponding column `c2`, the
